...
public class ScenarioManager implements OpenFilesHandler {
	...
	if (saved) {
		...
		final String save = CommonDialogs.showInputDialog("Open Which Saved Game?", title, saveList,
			saveList[0]);
		if (save != null) {
	...
	if (app.getMode() == Application.STATUS_GAME) {
		...
		final String extension = ScenarioManager.getExtension(this.lastUsedGameFile);
		if (extension != null) {
		    if (!extension.equals(Extension.getGameExtension())) {
	...
	while (!FilenameChecker.isFilenameOK(filename)) {
		...
		if (!FilenameChecker.isFilenameOK(filename)) {
		    ...
		    final String dir = SaveRegistration.getSaveBasePath() + File.separator;
	...
	for (File f : e.getFiles()) {
	    this.loadFromOSHandler(f.getAbsolutePath());
	}
    }
}
