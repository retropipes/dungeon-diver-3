...
public class GameLogic {
	...
	DungeonDiver3.getApplication().getScenarioManager().getMap().updateVisibleSquares(x, y, z);
    }
	...
	// Post-move actions
	final ArrayList<InternalScriptArea> areaScripts = app.getScenarioManager().getMap()
    ...
    public void updatePositionRelativeNoEvents(final int x, final int y, final int z) {
	boolean redrawsSuspended = false;
	final Map m = DungeonDiver3.getApplication().getScenarioManager().getMap();
	int px = m.getPlayerLocationX();
	int py = m.getPlayerLocationY();
	...
	final GenericCharacter player = (GenericCharacter) m.getCell(px, py, pz, MapConstants.LAYER_OBJECT);
	...
	try {
	    ...
	    m.setCell(new Player(), m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
		    MapConstants.LAYER_OBJECT);
	    DungeonDiver3.getApplication().showMessage("Can't go outside the map");
	} catch (final NullPointerException np) {
	    m.restorePlayerLocation();
	    this.getViewManager().restoreViewingWindow();
	    m.setCell(new Player(), m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
	...
	try {
	    ...
	    DungeonDiver3.getApplication().showMessage(gameName2 + " on " + gameName1);
	...
	if (this.stateChanged) {
	    ...
	    if (didMapExist) {
		app.getScenarioManager().getMap().setGeneratorRandomness(currRandom,
