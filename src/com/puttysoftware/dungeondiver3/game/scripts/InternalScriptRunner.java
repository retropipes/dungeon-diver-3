...
public final class InternalScriptRunner {
	...
	int actionCounter = 0;
	final Map m = DungeonDiver3.getApplication().getScenarioManager().getMap();
	try {
	    final GameLogic gm = DungeonDiver3.getApplication().getGameManager();
	    if (s != null) {
		...
		for (int x = 0; x < totAct; x++) {
			...
			final Shop shop = DungeonDiver3.getApplication().getGenericShop(shopType);
			if (shop != null) {
			    shop.showShop();
			} else {
			...
			final MapObject swapObj1 = DungeonDiver3.getApplication().getObjects()
				.getNewInstanceByName(swap1);
			final MapObject swapObj2 = DungeonDiver3.getApplication().getObjects()
				.getNewInstanceByName(swap2);
			DungeonDiver3.getApplication().getScenarioManager().getMap().findAllObjectPairsAndSwap(swapObj1,
				swapObj2);
			break;
		    case REDRAW:
			...
			DungeonDiver3.getApplication().getGameManager().addToScore(points);
			break;
		    case RANDOM_CHANCE:
			// Random Chance
		    ...
		    case BATTLE:
			// Hide the game
			DungeonDiver3.getApplication().getGameManager().hideOutput();
			// Battle
			...
			new Thread("Battle") {
				...
				try {
				    ...
				    DungeonDiver3.getApplication().getBattle()
					    .doFixedBattle(Map.getTemporaryBattleCopy(), battle);
				} catch (final Exception e) {
				    // Something went wrong in the battle
				    DungeonDiver3.logError(e);
				}
			    }
			}.start();
			break;
			...
			DungeonDiver3.getApplication().getGameManager().goToLevelRelative(rDestLevel);
			break;
		    default:
	    ...
	    final String beginMsg = "Buggy Internal Script, action #" + actionCounter + ": ";
	    final String endMsg = e.getMessage();
	    final String scriptMsg = beginMsg + endMsg;
	    DungeonDiver3.logWarning(new InternalScriptException(scriptMsg, e));
	}
    }

    private static void validateScriptEntry(final InternalScriptEntry se) {
	final InternalScriptActionCode code = se.getActionCode();
	final int rargc = InternalScriptConstants.ARGUMENT_COUNT_VALIDATION[code.ordinal()];
	int aargc;
	try {
	...
	final Class<?>[] rargt = InternalScriptConstants.ARGUMENT_TYPE_VALIDATION[code.ordinal()];
	if (rargt != null) {
