...
public class BattleLogic {
	...
	if (!this.resultDoneAlready) {
		...
		DungeonDiver3.getApplication().getGameManager().addToScore(Math.max(1,
	    ...
	    // Level Up Check
	    final ArrayList<InternalScript> levelUpScripts = PartyManager.getParty().checkPartyLevelUp();
	...
	if (this.bd.getActiveCharacter().getCurrentAP() > 0) {
	    ...
	    final ArrowTask at = new ArrowTask(x, y, this.bd.getActiveCharacter().getTemplate().getFaith().getFaithID(),
		    this.bd);
	    at.start();
	} else {
	...
	if (this.getActiveActionCounter() > 0) {
		...
		final boolean success = CombatItemChucker.selectAndUseItem(this.bd.getActiveCharacter().getTemplate(),
			this.bd.getActiveCharacter().getTeamID(), true, this.bd);
		if (success) {
		    this.bd.getActiveCharacter().modifyAP(BattleLogic.ITEM_ACTION_POINTS);
		}
		final int currResult = this.getResult();
		if (currResult != BattleResults.IN_PROGRESS) {
		    // Battle Done
		...
		// Active character has AI, and AI is turned on
		final CombatItem cui = this.bd.getActiveCharacter().getTemplate().getAI().getItemToUse();
	...
	if (this.getActiveActionCounter() > 0) {
	    ...
	    int stealChance;
	    int stealAmount = 0;
	    this.bd.getActiveCharacter().modifyAP(BattleLogic.STEAL_ACTION_POINTS);
