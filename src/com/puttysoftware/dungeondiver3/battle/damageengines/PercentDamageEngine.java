...
class PercentDamageEngine extends DamageEngine {
	...
	if (rHit > aHit) {
		...
		final Equipment mainHand = acting.getItems().getEquipmentInSlot(EquipmentSlotConstants.SLOT_MAINHAND);
		...
		final Equipment shirt = acting.getItems().getEquipmentInSlot(EquipmentSlotConstants.SLOT_UPPER_TORSO);
		if (shirt != null) {
		    for (int z = 0; z < fc; z++) {
		...
		final Equipment belt = acting.getItems().getEquipmentInSlot(EquipmentSlotConstants.SLOT_LOWER_TORSO);
		if (belt != null) {
		    for (int z = 0; z < fc; z++) {
