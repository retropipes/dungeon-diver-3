...
class BattleGUI {
	...
	this.battleFrame.setJMenuBar(DungeonDiver3.getApplication().getMenuManager().getMainMenuBar());
    }
	...
	if (this.battleFrame.isVisible()) {
		...
		for (y = yView; y <= ylView; y++) {
		    ...
		    yFix = y - yView;
		    try {
			final BufferedImageIcon icon1 = bd.getBattleMap().getBattleGround(y, x).battleRenderHook();
			final BufferedImageIcon icon2 = bd.getBattleMap().getBattleCell(y, x).battleRenderHook();
    ...
    void redrawOneBattleSquare(final BattleDefinitions bd, final int x, final int y, final MapObject obj3) {
	// Draw the battle, if it is visible
	if (this.battleFrame.isVisible()) {
	    try {
		int xFix, yFix;
		...
		xFix = y - xView;
		yFix = x - yView;
		final BufferedImageIcon icon1 = bd.getBattleMap().getBattleGround(x, y).battleRenderHook();
		final BufferedImageIcon icon2 = bd.getBattleMap().getBattleCell(x, y).battleRenderHook();
	...
	private void handleMovement(final KeyEvent e) {
		...
		final BattleLogic bl = DungeonDiver3.getApplication().getBattle();
