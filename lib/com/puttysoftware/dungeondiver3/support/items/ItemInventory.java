...
public class ItemInventory {
    ...
    void equipOneHandedWeapon(final Creature pc, final Equipment ei, final boolean useFirst, final boolean playSound) {
    ...
    void equipTwoHandedWeapon(final Creature pc, final Equipment ei, final boolean playSound) {
	// Fix character load, changing weapons
	if (this.equipment[EquipmentSlotConstants.SLOT_MAINHAND] != null) {
	    pc.offsetLoad(-this.equipment[EquipmentSlotConstants.SLOT_MAINHAND].getEffectiveWeight());
	}
    ...
    void equipArmor(final Creature pc, final Equipment ei, final boolean playSound) {
	// Check for socks
    ...
    public static ItemInventory readItemInventoryV1(final XDataReader dr) throws IOException {
	final ItemInventory ii = new ItemInventory(true);
	for (final ItemUseQuantity iqu : ii.entries) {
	    iqu.setQuantity(dr.readInt());
	    iqu.setUses(dr.readInt());
