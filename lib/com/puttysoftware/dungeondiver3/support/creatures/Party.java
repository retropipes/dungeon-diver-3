...
public class Party {
	...
	for (final BattleCharacter battler : this.battlers) {
		...
		act0.addActionArg(new InternalScriptEntryArgument(
			Math.max(1, (10 * battler.getTemplate().getLevel() - 1) / this.activePCs)));
	...
	for (int x = 0; x < divMod; x++) {
		...
		this.battlers[x].getTemplate()
			.offsetExperience(Creature.getAdjustedExperience(vsd.getExpPerMonster(y)) / divMod);
	    }
	...
	final String response = CommonDialogs.showInputDialog(
		"Pick " + number + " Party Member(s) - " + current + " of " + number, "DungeonDiver3", pickNames,
		pickNames[0]);
	if (response != null) {
	    final int loc = this.findMember(response, 0, this.members.length);
	    if (loc != -1) {
