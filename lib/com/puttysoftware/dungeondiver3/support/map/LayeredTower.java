...
class LayeredTower implements Cloneable {
	...
	do {
	    ...
	    try {
		final MapObject obj = this.getCell(fx1, fy1, zLoc, MapConstants.LAYER_OBJECT);
    ...
    private void fillFloor(final MapObject bottom, final MapObject top, final int z) {
	int x, y, e;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
		...
		for (x = 0; x < columns; x++) {
			...
			if (e == MapConstants.LAYER_GROUND) {
				...
				for (y = 0; y < rows; y += this.regionSize) {
				    ...
				    final boolean okay = placeObj.shouldGenerateObject(map, y, x, z, w, e);
				    if (okay) {
	...
	final int rows = this.getRows();
	for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
	    final MapObject[] objectsWithoutPrerequisites = objects.getAllNotRequiredInBattle(e);
	    if (objectsWithoutPrerequisites != null) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
	...
	// Pass 3
	for (int layer = 0; layer < MapConstants.LAYER_COUNT; layer++) {
	    final MapObject[] requiredObjects = objects.getAllRequiredInBattle(layer);
	    if (requiredObjects != null) {
		final RandomRange row = new RandomRange(0, this.getRows() - 1);
		final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		int randomColumn, randomRow;
    ...
    public ArrayList<InternalScriptArea> getScriptAreasAtPoint(final Point p, final int z) {
    ...
    public static LayeredTower readXLayeredTower(final XDataReader reader, final int ver) throws IOException {
	final MapObjectList objects = new MapObjectList();
